package com.aron.framedemo.ui.adapter;
...
import com.aron.framedemo.listener.OnItemClickListener;
import com.aron.framedemo.ui.adapter.base.BaseRecyclerViewAdapter;
import com.aron.framedemo.util.DimenUtil;
...
public class NewsListAdapter extends BaseRecyclerViewAdapter<NewsSummary> {
    ...
    private void setItemValues(ItemViewHolder holder, int position) {
        ...
        if (holder.mNewsSummaryTitleTv == null){
        }
        ...
        Glide.with(BaseApplication.getAppContext()).load(imgSrc).asBitmap() // gif格式有时会导致整体图片不显示，貌似有冲突
    ...
    private void setImageView(PhotoViewHolder holder, NewsSummary newsSummary) {
        ...
        if (newsSummary.getAds() != null) {
            ...
            if (size >= 3) {
                ...
                holder.mNewsSummaryTitleTv.setText(BaseApplication.getAppContext()
    ...
    private void showAndSetPhoto(ImageView imageView, String imgSrc) {
        ...
        Glide.with(BaseApplication.getAppContext()).load(imgSrc).asBitmap()
                .format(DecodeFormat.PREFER_ARGB_8888)
                .diskCacheStrategy(DiskCacheStrategy.ALL)
                .placeholder(R.color.image_place_holder)
                .error(R.drawable.ic_load_fail)
